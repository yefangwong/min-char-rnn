...
This will train the model and save the weights to a file (e.g., `rnn_model_64000.dat`) in the project's root directory.
...
- `<model_path>`: Path to the `.dat` model file (e.g., `rnn_model_64000.dat`).
...
To generate sequences such as "鮭魚生魚片" (5 characters starting with "鮭"), or "生魚片" (3 characters starting with "生"), ensure `rnn_model_64000.dat` is in the root directory and run:

```bash
java -cp out SimpleRNN --inference rnn_model_64000.dat 鮭 4
```
